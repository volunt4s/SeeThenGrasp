...
from recon_src.coarse_search.lib import utils, dvgo
...
def raw_image_process(current_image,
    ...
    bbox_xyxy = detection_result[0].boxes[0].xyxy.detach().cpu().numpy().astype(int).squeeze()
...
def scene_rep_reconstruction(args, cfg, cfg_model, cfg_train, xyz_min, xyz_max, data_dict, stage, coarse_ckpt_path=None):
    ...
    # get pre-generated apriltag cube information (Stage 1)
    ...
    if active_learning:
        ...
        pose_param = autil.init_opt_model(
            init_theta=torch_uniform_sample(torch.pi/6, torch.pi/3),
            init_phi=torch_uniform_sample((torch.pi+torch.pi/8), (2*torch.pi-torch.pi/8)),
            radius=radius)
    ...
    original_img_path = os.path.join("generated_data", "image")
    mask_path = os.path.join("generated_data", "mask")
    rmbg_path = os.path.join("generated_data", "image_rmbg")
    ...
    initial_image = image_saver.get_current_image(camera='camera')    
    masked_img, processed_mask, original_img = raw_image_process(initial_image,
                                                                 yolo_model,
                                                                 sam_generator)
    init_pose = cam1_cube_info['init_ext']
    i_train = [0] # set initial train image
    images = torch.from_numpy(masked_img)[None, ...].float()
    poses = torch.from_numpy(init_pose)[None, ...].float().to(device)
    ...
    for global_step in trange(1+start, 1+cfg_train.N_iters):
        ...
        if active_learning:
            if global_step % active_add_step == 0 and global_step != 0:
                ...
                to_go_joint_value = cam1_joint_info[to_go_joint_key]
                ...
                if reached:
                    # capture image and post processing (hand-eye camera)
                    image_saver = ImageSaver()
                    current_image = image_saver.get_current_image(camera='camera')
                    masked_img, processed_mask, original_img = raw_image_process(current_image,
                                                                                yolo_model,
                                                                                sam_generator)
                    pose_to_add = np.linalg.inv(cam1_cube_info[cube_closest_key])
                    pose_to_out.append(pose_to_add)

                    # save images to perform fine optimization
                    if active_cnt < 10:
                        current_image = cv2.cvtColor(current_image, cv2.COLOR_BGR2RGB)
                        cv2.imwrite(os.path.join(mask_path, f"00{active_cnt}.png"), processed_mask * 255)
                        cv2.imwrite(os.path.join(original_img_path, f"00{active_cnt}.png"), current_image)
                    else:
                        current_image = cv2.cvtColor(current_image, cv2.COLOR_BGR2RGB)
                        cv2.imwrite(os.path.join(mask_path, f"0{active_cnt}.png"), processed_mask * 255)
                        cv2.imwrite(os.path.join(original_img_path, f"0{active_cnt}.png"), current_image)
                    
                    # actively add to current train dataset
                    to_add_index = active_cnt + 1
    ...
    # Stage 1 end => save data
    pose_to_out = np.array(pose_to_out)
    np.save("generated_data/nonpick_search.npy", pose_to_out)
...
def active_add_dataset_model(original_train_poses,
                             model,
                             pose_param,
                             opt_step,
                             render_kwargs,
    ...
    for i in range(opt_step):
        # construct differential pose via spherical parameter
        ...
        with torch.no_grad():
            theta, phi = spherical_model()
            theta.clamp_(torch.pi/5, torch.pi/3) 
            phi.clamp_((torch.pi+torch.pi/6), (2*torch.pi-torch.pi/6))

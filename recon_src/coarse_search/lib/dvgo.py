...
class DirectVoxGO(torch.nn.Module):
    def __init__(self, xyz_min, xyz_max,
        ...
        self.entropy_inferencing = False
        ...
        self.density = torch.nn.Parameter(torch.ones([1, 1, *self.world_size]))

        # init entropy voxel
        self.entropy_volume = torch.ones([1, 1, *self.world_size]) * 0.5

        # IMPORTANT!
        #   For stage 1, we reduced uncertainty element except object area in stage 1
        #   this version can take more precise uncertainty
        self.entropy_volume[0, 0, :, :20, :] = 0.0
        self.entropy_volume[0, 0, :, 50:, :] = 0.0
        self.entropy_volume[0, 0, :, :, 50:] = 0.0
        self.entropy_volume[0, 0, :, :, :25] = 0.0
        self.entropy_volume[0, 0, 75:, :, :] = 0.0
        self.entropy_volume[0, 0, :35, :, :] = 0.0
    ...
    def forward(self, rays_o, rays_d, viewdirs, global_step=None, **render_kwargs):
        ...
        rays_pts, mask_outbbox = self.sample_ray(
                rays_o=rays_o, rays_d=rays_d, is_train=global_step is not None, **render_kwargs) # (N_rays, N_sample), (N_ray, N_sample)
        ...
        ent_alpha = torch.zeros_like(rays_pts[..., 0])
        ...
        # query for entropy volume
        # ent_alpha[~mask_outbbox] = self.grid_sampler(rays_pts[~mask_outbbox], self.entropy_volume)
        ent_alpha = self.grid_sampler(rays_pts, self.entropy_volume)
        ...
        # Entropy inferencing
        if self.entropy_inferencing:
            if ent_alpha.ndim > 2:
                ent_alpha = ent_alpha.view(-1, ent_alpha.shape[-1])
                alpha = alpha.view(-1, alpha.shape[-1])
                mask_outbbox = mask_outbbox.view(-1, mask_outbbox.shape[-1])
            entropy = self.get_entropy_of_ray(ent_alpha, version="ActiveRMAP")
            alpha_entropy = self.get_entropy_of_ray(alpha, version="ActiveRMAP")
        else:
            entropy = None
            alpha_entropy = None
        ...
        ret_dict.update({
            ...
            'alpha_grid': self.activate_density(self.density, interval),
            'entropy_volume': self.entropy_volume
        })
        return ret_dict

    def get_entropy_of_ray(self, alpha, version):
        if version == "ActiveRMAP":
            entropy = -alpha * torch.log(alpha + 1e-10) - (1. - alpha) * torch.log(1. -alpha + 1e-10)
            entropy = torch.sum(entropy, dim=-1)
        elif version == "InfoNeRF":
            ray_prob = alpha / (torch.sum(alpha, dim=-1).unsqueeze(-1) + 1e-10)
            entropy = -torch.sum(ray_prob * torch.log(ray_prob + 1e-10) + (1 - ray_prob) * torch.log(1 - ray_prob + 1e-10), dim=-1)
        return entropy
    
    @torch.no_grad()
    def synchronize_entropy_volume(self, indices, **render_kwargs):
        interval = render_kwargs['stepsize'] * self.voxel_size_ratio
        alpha_volume = self.activate_density(self.density, interval)
        # self.entropy_volume[indices] /= 2
        self.entropy_volume[indices] = alpha_volume[indices]
...
def get_rays_center(H, W, K, c2w, inverse_y, flip_x, flip_y, mode='center'):
    half_H, half_W = H // 2, W // 2
    
    start_i = (W - half_W) // 2
    end_i = start_i + half_W
    
    start_j = (H - half_H) // 2
    end_j = start_j + half_H
    
    i, j = torch.meshgrid(
        torch.linspace(start_i, end_i-1, half_W, device=c2w.device),
        torch.linspace(start_j, end_j-1, half_H, device=c2w.device))  # pytorch's meshgrid has indexing='ij'
...
def get_rays_of_a_view_center(H, W, K, c2w, ndc, inverse_y, flip_x, flip_y, mode='center'):
    rays_o, rays_d = get_rays_center(H, W, K, c2w, inverse_y=inverse_y, flip_x=flip_x, flip_y=flip_y, mode=mode)
    viewdirs = rays_d / rays_d.norm(dim=-1, keepdim=True)
    if ndc:
        rays_o, rays_d = ndc_rays(H, W, K[0][0], 1., rays_o, rays_d)
    return rays_o, rays_d, viewdirs

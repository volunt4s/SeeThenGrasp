...
def create_optimizer_or_freeze_model_ensemble(ensemble_model, cfg_train, global_step):
    decay_steps = cfg_train.lrate_decay * 1000
    decay_factor = 0.1 ** (global_step/decay_steps)

    param_group = []
    for one_model in ensemble_model:
        for k in cfg_train.keys():
            if not k.startswith('lrate_'):
                continue
            k = k[len('lrate_'):]

            if not hasattr(one_model, k):
                continue

            param = getattr(one_model, k)
            if param is None:
                print(f'create_optimizer_or_freeze_model: param {k} not exist')
                continue

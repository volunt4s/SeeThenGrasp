...
def render_viewpoints(model, render_poses, HW, Ks, ndc, render_kwargs,
    ...
    os.makedirs(os.path.join(savedir, "normal"), exist_ok=True)
    rgbs = []
    normals = []
    ins = []
    outs = []
    ...
    theta_range = np.linspace(np.pi/2-np.pi/2.5, np.pi/2+np.pi/2.5, 75)
    phi_range = np.linspace(0, np.pi, 75)

    theta_range2 = np.linspace(np.pi/2+np.pi/2.5, np.pi/2-np.pi/2.5, 75)
    phi_range2 = np.linspace(np.pi, 2*np.pi, 75)

    render_poses = []

    for i in range(len(theta_range)):
        render_poses.append(generate_cam_pose(0.5, theta_range[i], phi_range[i]))

    for i in range(len(theta_range)):
        render_poses.append(generate_cam_pose(0.5, theta_range2[i], phi_range2[i]))
    ...
    for i, c2w in enumerate(tqdm(render_poses)):        
        ...
        if savedir is not None:
            ...
            if render_normal:
                ...
                step_pre = str(step) + '_' if step > 0 else ''
                filename = os.path.join(savedir, "normal", step_pre+'{:03d}_normal.png'.format(id))
...
def validate_mesh(model, resolution=512, threshold=0.0, prefix="", world_space=False,
    ...
    print()
    mesh_path = os.path.join(cfg.basedir, cfg.expname, "meshes", "mesh.ply")
...
def scene_rep_reconstruction(args, cfg, cfg_model, cfg_train, xyz_min, xyz_max, data_dict, stage, coarse_ckpt_path=None, use_dvgo=False):
    ...
    logger.info(f"{xyz_min} // {xyz_max}")
    margin_bound_min = torch.Tensor([0.01, 0.01, 0.01])
    margin_bound_max = torch.Tensor([0.01, 0.01, 0.01])
    xyz_min = xyz_min - margin_bound_min
    xyz_max = xyz_max + margin_bound_max
    logger.info(f"{xyz_min} // {xyz_max}")
...
if __name__=='__main__':
    ...
    export_bbox_and_cams_only = False
    if export_bbox_and_cams_only:
        ...
        np.savez_compressed(args.export_bbox_and_cams_only,
                            ...
                            cam_lst=np.array(cam_lst), pose_lst=np.array(pose_lst))
        logger.info('done')
        sys.exit()
    ...
    if args.mesh_from_sdf:
        ...
        with torch.no_grad():
            ...
            validate_mesh(model, 512, threshold=0.001, prefix="{}final_mesh".format(prefix), world_space=True,
    ...
    if args.render_test or args.render_train or args.render_video or args.interpolate or args.render_poses:
        if args.ft_path:
    ...
    if args.render_poses:
        testsavedir = os.path.join(cfg.basedir, cfg.expname, f'render_poses_{ckpt_name}')
        os.makedirs(testsavedir, exist_ok=True)
        rgbs, disps = render_viewpoints(
